...
generate_addressability_pool <- function(transaction_df)
{
  ...
  BIC <- transaction_df %>% select(official_bic_contract) %>% na.omit() %>% collect() %>% .$official_bic_contract
  ...
  for(i in 1:length(BIC))
  {
    ...
    list_of_psc_naics <- transaction_df %>% filter(official_bic_contract == BIC[i]) %>% distinct(product_or_service_code, naics_code)%>%
