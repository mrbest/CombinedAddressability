...
load_spark_csv <- function(sc, filename, raw_arch_out)
...
#write complete FPDS archive out
  spark_write_parquet(raw_df, raw_arch_out)
  raw_df
}


socio_economic_factor_cleanup <- function(raw_df)
{
  print("Performing socio-economic factor clean-up")
  ###Re-code NAs first!!!!!!!
  raw_df <- raw_df %>% mutate(product_or_service_code = if_else(is.na(product_or_service_code) == TRUE, "FALSE", product_or_service_code))
  raw_df <- raw_df %>% mutate(naics_code = if_else(is.na(naics_code) == TRUE, "FALSE", naics_code))
...
{
  print("Reading in export")
  ld_df <- spark_read_parquet(sc, name="ld_df", path = archive)
...
set_training_date_range <- function(df, start_date, end_date)
{
  training_transactions <- df %>% filter(as.Date(date_signed) >= as.Date(start_date) & as.Date(date_signed) <= as.Date(end_date)) 
  training_transactions <- training_transactions %>% mutate(psc_naics_key = paste0(product_or_service_code,"_",naics_code))
  training_transactions <- training_transactions %>% mutate(contract_availability_key = paste0(product_or_service_code,"_",naics_code,"_", sbg_flag,"_", women_owned_flag,"_", veteran_owned_flag,"_", minority_owned_business_flag,"_", foreign_government, "_", co_bus_size_determination_code, "_", foreign_funding_desc, "_", firm8a_joint_venture, "_", dot_certified_disadv_bus, "_", sdb, "_", sdb_flag, "_", hubzone_flag, "_", sheltered_workshop_flag,"_", srdvob_flag, "_", other_minority_owned, "_", baob_flag, "_", aiob_flag, "_", naob_flag, "_", haob_flag, "_", saaob_flag, "_", emerging_small_business_flag, "_", wosb_flag, "_", edwosb_flag, "_", jvwosb_flag, "_", edjvwosb_flag))
  training_transactions
}

set_testing_date_range <- function(df, start_date, end_date)
{
  testing_transactions <- df %>% filter(as.Date(date_signed) >= as.Date(start_date) & as.Date(date_signed) <= as.Date(end_date)) 
  testing_transactions <- testing_transactions %>% mutate(psc_naics_key = paste0(product_or_service_code,"_",naics_code))
  testing_transactions <- testing_transactions %>% mutate(contract_availability_key = paste0(product_or_service_code,"_",naics_code,"_", sbg_flag,"_", women_owned_flag,"_", veteran_owned_flag,"_", minority_owned_business_flag,"_", foreign_government, "_", co_bus_size_determination_code, "_", foreign_funding_desc, "_", firm8a_joint_venture, "_", dot_certified_disadv_bus, "_", sdb, "_", sdb_flag, "_", hubzone_flag, "_", sheltered_workshop_flag,"_", srdvob_flag, "_", other_minority_owned, "_", baob_flag, "_", aiob_flag, "_", naob_flag, "_", haob_flag, "_", saaob_flag, "_", emerging_small_business_flag, "_", wosb_flag, "_", edwosb_flag, "_", jvwosb_flag, "_", edjvwosb_flag)) 
  testing_transactions
}

create_mini_from_df <- function(ld_df, arch_out_name)
{

  mini_df <-  ld_df %>% select(award_type_description, co_bus_size_determination_code, date_signed, eight_a_flag, funding_agency_name, funding_department_name,
                               minority_owned_business_flag, naics_code, naics_description, piid, idv_ref_idv_piid, product_or_service_code, 
                               reference_piid, small_business_flag, level_1_category, veteran_owned_flag, women_owned_flag, psc_naics_key, contract_availability_key,
                               ...
                               emerging_small_business_flag, wosb_flag, edwosb_flag, jvwosb_flag, contract_name, managing_agency, business_rule_tier, edjvwosb_flag, 
                               level_1_category_group, funding_cfo_act_agency, sheltered_workshop_flag, pop_country_code, vendor_duns_number, contingency_ops, 
                               funding_agency_code, reason_not_competed, contracting_office_id, funding_agency_code, dollars_obligated, type_of_set_aside, hbcu_flag,
                               minority_institution_flag, official_bic_flag, funding_cfo_act_agency)
  
  
  print("writing out mininal parquet file")
  spark_write_parquet(mini_df, arch_out_name)
  mini_df
}
...
sparkRLoadGWCMParquet <- function()
{
  ...
  training_transactions$addkey <<- SparkR::concat_ws(sep = "_", training_transactions$product_or_service_code,training_transactions$naics_code, training_transactions$sbg_flag, training_transactions$women_owned_flag, training_transactions$veteran_owned_flag, training_transactions$minority_owned_business_flag, training_transactions$foreign_government) 
  training_transactions$psc_naics_key <<- SparkR::concat_ws(sep = "_", training_transactions$product_or_service_code,training_transactions$naics_code)
  training_transactions$case_multikey <<- SparkR::concat_ws(sep = "_",training_transactions$product_or_service_code,training_transactions$naics_code, training_transactions$sbg_flag, training_transactions$women_owned_flag, training_transactions$veteran_owned_flag, training_transactions$minority_owned_business_flag, training_transactions$foreign_government,  training_transactions$co_bus_size_determination_code,  training_transactions$foreign_funding_desc,  training_transactions$firm8a_joint_venture,  training_transactions$dot_certified_disadv_bus,  training_transactions$sdb,  training_transactions$sdb_flag,  training_transactions$hubzone_flag,  training_transactions$sheltered_workshop_flag, training_transactions$srdvob_flag,  training_transactions$other_minority_owned,  training_transactions$baob_flag,  training_transactions$aiob_flag,  training_transactions$naob_flag,  training_transactions$haob_flag,  training_transactions$saaob_flag,  training_transactions$emerging_small_business_flag,  training_transactions$wosb_flag,  training_transactions$edwosb_flag,  training_transactions$jvwosb_flag,  training_transactions$edjvwosb_flag)
  ...
  testing_transactions$addkey <<- SparkR::concat_ws(sep = "_", training_transactions$product_or_service_code,training_transactions$naics_code, training_transactions$sbg_flag, training_transactions$women_owned_flag, training_transactions$veteran_owned_flag, training_transactions$minority_owned_business_flag, training_transactions$foreign_government) 
  testing_transactions$psc_naics_key <<- SparkR::concat_ws(sep = "_", training_transactions$product_or_service_code,training_transactions$naics_code)
  testing_transactions$case_multikey <<- SparkR::concat_ws(sep = "_",training_transactions$product_or_service_code,training_transactions$naics_code, training_transactions$sbg_flag, training_transactions$women_owned_flag, training_transactions$veteran_owned_flag, training_transactions$minority_owned_business_flag, training_transactions$foreign_government,  training_transactions$co_bus_size_determination_code,  training_transactions$foreign_funding_desc,  training_transactions$firm8a_joint_venture,  training_transactions$dot_certified_disadv_bus,  training_transactions$sdb,  training_transactions$sdb_flag,  training_transactions$hubzone_flag,  training_transactions$sheltered_workshop_flag, training_transactions$srdvob_flag,  training_transactions$other_minority_owned,  training_transactions$baob_flag,  training_transactions$aiob_flag,  training_transactions$naob_flag,  training_transactions$haob_flag,  training_transactions$saaob_flag,  training_transactions$emerging_small_business_flag,  training_transactions$wosb_flag,  training_transactions$edwosb_flag,  training_transactions$jvwosb_flag,  training_transactions$edjvwosb_flag)
  
}

sparkRLoadRAWParquet <- function()
{
  ld_df <- read.df("../CombinedAddressability/04DEC17FY13_17.prq/")
  raw_df <<- ld_df %>% SparkR::filter(ld_df$level_1_category_group == "GWCM")
  print("subsetting training transactions")
